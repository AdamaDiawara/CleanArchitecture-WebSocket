...
export default function RestaurantsPage() {
  ...
  const handleToggleStatus = async (restaurant: RestaurantDto) => {
    ...
    if (result.ok && result.data) {
      setRestaurants((previous) =>
        previous.map((item) => item.id === restaurant.id ? result.data! : item),
      );
    } else {
  ...
  const handleSaveProfile = async (restaurantId: string) => {
    ...
    if (result.ok && result.data) {
      setRestaurants((previous) =>
        previous.map((item) => item.id === restaurantId ? result.data! : item),
      );
          ...
          <div className="p-6 pb-4">
            <div className="flex items-start justify-between gap-4">
              ...
              <div className="flex items-center gap-2 shrink-0">
                <Link
                  href={`/dashboard/restaurants/menu?restaurantId=${restaurant.id}`}
            ...
            {editingProfileId === restaurant.id && (
              <div className="mt-4 space-y-3 border-t border-slate-100 pt-4">
                ...
                <div className="flex gap-3">
                  <button type="button"
                    onClick={() => handleSaveProfile(restaurant.id)}
            ...
            {editingHoursId === restaurant.id ? (
              <div className="space-y-4">
                ...
                <div className="flex gap-3">
                  <button
                    type="button"
                    onClick={() => handleSaveHours(restaurant.id)}
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="col-span-2">
                ...
                <input type="text" required value={createForm.name}
                  onChange={(changeEvent) => setCreateForm((previousForm) => ({ ...previousForm, name: changeEvent.target.value }))}
              ...
              <div className="col-span-2">
                ...
                <input type="text" required value={createForm.address}
                  onChange={(changeEvent) => setCreateForm((previousForm) => ({ ...previousForm, address: changeEvent.target.value }))}
                  className="w-full rounded-xl border border-slate-200 px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-orange-400"
                />
              </div>
              <div>
                ...
                <input type="text" required value={createForm.cuisineType}
                  onChange={(changeEvent) => setCreateForm((previousForm) => ({ ...previousForm, cuisineType: changeEvent.target.value }))}
              ...
              <div>
                ...
                <input type="number" required min={5} value={createForm.prepTimeMin}
                  onChange={(changeEvent) => setCreateForm((previousForm) => ({ ...previousForm, prepTimeMin: Number(changeEvent.target.value) }))}
                  className="w-full rounded-xl border border-slate-200 px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-orange-400"
                />
              </div>
              <div>
                ...
                <input type="number" min={0} step={0.5} value={createForm.deliveryFee}
                  onChange={(changeEvent) => setCreateForm((previousForm) => ({ ...previousForm, deliveryFee: Number(changeEvent.target.value) }))}
                  className="w-full rounded-xl border border-slate-200 px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-orange-400"
                />
              </div>
            </div>
