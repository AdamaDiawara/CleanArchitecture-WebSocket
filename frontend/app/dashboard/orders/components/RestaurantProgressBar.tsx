...
export function RestaurantProgressBar({ status, hasDriver }: { status: string; hasDriver: boolean }) {
  const currentIndex = (() => {
    if (status === "created")                  return 0; // Reçue
    if (status === "confirmed")                return 2; // En préparation (peu importe si livreur assigné)
    if (status === "prepared" && !hasDriver)   return 3; // Prête — en attente d'un livreur
    if (status === "prepared" && hasDriver)    return 4; // Remise au livreur
    if (status === "delivering" || status === "delivered") return 6;
    return 0;
  })();

  const steps = ["Reçue", "Confirmée", "En préparation", "Prête", "Remise livreur", "Récupérée"];
